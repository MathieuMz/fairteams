...
export async function register(app: FastifyInstance) {
  ...
  // GET /competitions/:slug — retourne competition + players + constraints + snapshots
  app.get('/competitions/:slug', async (req, reply) => {
    ...
    const [players, constraints, snapshots] = await Promise.all([
      repo.listPlayers(competition.id),
      repo.listConstraints(competition.id),
      repo.listSnapshots(competition.id),
    ])
    return { competition, players, constraints, snapshots }
  })
