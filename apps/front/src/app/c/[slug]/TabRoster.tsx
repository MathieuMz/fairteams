...
export default function TabRoster({ players, competition, onUpdated }: Props) {
  ...
  async function applyChanges() {
    setSaving(true)
    try {
      await Promise.all(Object.entries(patches).map(([id, p]) => api.updatePlayer(id, p)))
      setPatches({})
      onUpdated()
    } finally {
      setSaving(false)
  ...
  return (
    <div>
      {dirty && (
        <div className="flex items-center gap-3 p-3 rounded-xl mb-3 text-sm" style={{ background: 'var(--warn-tint)', color: 'var(--warn)' }}>
          <span className="flex-1">Des modifications ne sont pas encore appliquées.</span>
      ...
      <div className="rounded-xl border" style={{ background: 'var(--surface)', borderColor: 'var(--border)' }}>
          ...
          return (
            <div key={p.id} className="flex items-center gap-3 px-4 py-3 text-sm"
              ...
              <span className="flex items-center gap-1 text-xs" style={{ color: 'var(--text-2)' }}>
                niv.
                <input type="number" step={1} value={level}
                  className="w-12 rounded border px-2 py-0.5 text-xs"
                  style={inputStyle}
                  onChange={e => patch(p.id, { level: parseInt(e.target.value) || 0 })} />
              </span>
              <label className="flex items-center gap-1.5 text-xs cursor-pointer" style={{ color: 'var(--text-2)' }}>
                <input type="checkbox" checked={isCaptain}
                  onChange={e => patch(p.id, { isCaptain: e.target.checked })} />
                capitaine
              </label>
              <select value={team === null || team === undefined ? '' : String(team)}
                className="rounded-lg border px-2 py-1 text-xs"
                style={inputStyle}
                onChange={e => patch(p.id, { team: e.target.value === '' ? null : parseInt(e.target.value) })}>
        ...
        {filtered.length === 0 && (
          <p className="text-sm text-center py-8" style={{ color: 'var(--text-2)' }}>Aucun joueur trouvé</p>
        )}
      </div>
    </div>
  )
