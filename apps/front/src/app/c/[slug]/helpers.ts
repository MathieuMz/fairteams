...
export function buildDemoPlayers(startIdx: number, count: number) {
  return Array.from({ length: count }, (_, i) => {
    ...
    const firstName =
      gender === "H"
        ? DEMO_MALE[idx % DEMO_MALE.length]
        : DEMO_FEMALE[idx % DEMO_FEMALE.length];
    const lastName = DEMO_SURNAMES[(idx * 7) % DEMO_SURNAMES.length];
    const level = Math.max(
      1,
      Math.min(100, Math.round(1 + Math.random() * 99)),
    );
    return {
      firstName,
      lastName,
...
export function bestTeamForPlayerFront(
  ...
  for (let t = 0; t < cfg.numTeams; t++) {
    ...
    const avgDiff =
      stats.members.length > 0 ? Math.abs(stats.avg - player.level) : 0;
